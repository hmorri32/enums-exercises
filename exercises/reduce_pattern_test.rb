...
class ReducePatternTest < Minitest::Test
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    ...
    keywords.each {|word| phrase = phrase.gsub(word, word.upcase)}
  ...
  def test_divide_560_by_a_bunch_of_numbers
    numbers  = [2, 2, 2, 5, 7]
    quotient = 560
    numbers.each {|number| quotient /= number}
  ...
  def test_subtract_smallest_values_from_100
    ...
    difference = 100
    elements.each {|element| difference -= element.min}
  ...
  def test_add_all_the_second_values_together
    ...
    sum      = 0
    elements.each {|element| sum += element[1]}
    assert_equal 31, sum
  end
end